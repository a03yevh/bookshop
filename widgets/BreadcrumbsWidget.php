    ...
    class BreadcrumbsWidget extends Breadcrumbs
    {
        ...
        protected function renderItemMarkup($link, $template, $position)
        {
            ...
            if (array_key_exists('label', $link)) {
                $label = Html::tag('span', $encodeLabel ? Html::encode($link['label']) : $link['label'], ['class' => 'breadcrumb__item-name', 'itemprop' => "name"]);
            } else {
            ...
            if (isset($link['url'])) {
                ...
                $link = Html::a($label, $link['url'], array_merge($options, ["class" => "breadcrumb__item-link", "itemprop" => "item"]));
            } else {
                $link = Html::tag('span', $label, ["class" => "breadcrumb__item-name", "itemprop" => "item"]);
            }
